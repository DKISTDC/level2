...
buttonLayer st =
 ...
 where
  ...
  buttonData =
    [ (Button1, "Button 1", attrName "button1")
...
aMap =
  attrMap
    V.defAttr
    [ (attrName "info", V.black `on` V.magenta)
    , (attrName "button1", V.black `on` V.cyan)
    , (attrName "button2", V.black `on` V.green)
    , (attrName "button3", V.black `on` V.blue)
