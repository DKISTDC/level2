...
page :: (Hyperbole :> es, Reader App :> es, Globus :> es) => Page es '[]
page = do
  UserSession user _ <- lookupUser
  ...
  case user of
    ...
    (Just _) -> redirect $ routeUri Dashboard
    -- need an admin login
    Nothing -> pure $ layout $ do
...
instance (Log :> es, Reader App :> es, Globus :> es, Time :> es) => HyperView AuthRed es where
  data Action AuthRed
