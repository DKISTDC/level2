...
import App.Page.Program (ProgramDatasets (..), viewDatasets, viewProgramDetails)
...
page
  ...
  => Id Proposal
  -> Page es '[ProgramSummary, ProgramDatasets]
page propId = do
...
viewProgramSummary l1 srt now prog ds = do
  ...
  programCard $ do
    ...
    col ~ pad (TRBL 0 15 15 15) $ do
      hyper (ProgramDatasets propId progId) $ viewDatasets l1 (NE.toList ds) Latest srt
