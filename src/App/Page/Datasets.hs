...
viewSyncSummary s = do
 ...
 where
  ...
  scanProgress ss (Just ps)
    ...
    | otherwise = do
        el $ text $ cs $ show (length s.scans)
        text " / "
        el $ text $ cs $ show (length s.proposals)
  ...
  col ~ gap 10 $ do
    forM_ gps $ \g -> do
      ...
      row ~ gap 10 $ do
        ...
        text " - "
        el $ text $ cs $ show (length g.items) <> " datasets"
...
errorsTable errs = do
 ...
 where
  errorRow (ScanError err val) = do
    ...
    code (cs $ A.encode val) ~ bg (light Light) . fontSize 12 . wrap . pad 4
  wrap =
    utility
      "wrap"
...
section :: (Styleable h) => CSS h -> CSS h
