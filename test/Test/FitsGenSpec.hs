...
specWCS = describe "WCS" $ do
  ...
  aroundAll slitX $ describe "binning adjustment" $ do
    itWithOuter "should preserve CRVAL" $ \w -> do
      w.crval `shouldBe` Key 100

    itWithOuter "should scale CDELT up. fewer pixels = higher delta " $ \w -> do
      w.cdelt `shouldBe` Key 1

    itWithOuter "should scale CRPIX down, since we have fewer pixels" $ \w -> do
      w.crpix `shouldSatisfy` isKeyApprox 100
 where
  isKeyApprox v (Key t) = abs (v - t) < 1

  slitX m = do
    w <- runErrorIO $ do
      (x, _) <- requireWCSAxes keys_2_114
      wcsSlitX @WCSMain x (BinnedX 200) keys_2_114
    m w
